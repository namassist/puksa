...
const KoalaAscii = ({ isBlinking }: { isBlinking: boolean }) => {
  // Simple ASCII koala with clear eye difference
  const koalaLines = [
    "       .--.       ",
    isBlinking ? "      | -_- |      " : "      |o_o |      ",
    "      |:_/ |      ",
...
export const WelcomeOutput = () => {
  ...
  const isBlinking = useBlinkingEyes();
  ...
  return (
    <div className="space-y-4">
      <pre className={`text-lavender text-sm leading-tight font-mono transition-all duration-500 ${showAscii ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'}`}>
        {KoalaAscii({ isBlinking })}
      </pre>
